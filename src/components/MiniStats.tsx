...
export function MiniStats() {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 border border-stone-line divide-x divide-stone-line">
      {tiles.map((t) => (
        <div key={t.label} className="p-4">
          ...
          <span className="font-data text-[10px] text-ink-faint">{t.note}</span>
        </div>
      ))}
      <Link
        href="/pool"
        className="col-span-2 sm:col-span-4 border-t border-stone-line px-4 py-2 font-data text-[10px] uppercase tracking-wider text-ink-faint hover:text-bronze-text transition-colors"
      >
